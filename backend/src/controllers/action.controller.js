...
function parseActionTimeRange(searchValue) {
  ...
  if (!match) {
    return {
      ok: false,
      message: 'Action Time must follow DD/MM/YYYY with optional HH, HH:mm or HH:mm:ss',
    };
  }
...
async function getActions(req, res) {
  ...
  try {
    ...
    if (normalizedDevice) {
      const selectedDevice = await Device.findOne({ name: normalizedDevice }).select('_id').lean();

      if (!selectedDevice?._id) {
        return res.json({ total: 0, page, limit, data: [], deviceList: PUBLIC_DEVICE_LIST });
      }

      andConditions.push({ idDevice: selectedDevice._id });
    }
    ...
    const items = docs.map((doc) => {
      ...
      return {
        id: doc._id,
        sensor_name: getDeviceLabel(deviceName),
    ...
    return res.json({ total, page, limit, data: items, deviceList: PUBLIC_DEVICE_LIST });
  } catch (error) {
